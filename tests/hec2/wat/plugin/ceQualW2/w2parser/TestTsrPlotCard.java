...
public class TestTsrPlotCard {

    public void unitTest(String folder) throws IOException {
        String infile = String.format("data/%s/w2_con.npt", folder);
        W2ControlFile w2con = new W2ControlFile(infile);
        TsrPlotCard card = new TsrPlotCard(w2con);
        String TSRC = card.getTSRC();
        int NTSR = card.getNTSR();
        int NITSR = card.getNITSR();
        card.setTSRC("TEST");
        card.setNTSR(991);
        card.setNITSR(992);
        card.updateDataTable();
        card.updateW2ControlFileList();
        w2con.save(String.format("results/%s/TestTsrPlotCard_w2_con.npt", folder));
    }
